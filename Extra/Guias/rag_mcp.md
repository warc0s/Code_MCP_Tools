...
Note: the app splits persistence across two DBs: DuckDB for RAG and SQLite for memory (`projects/items`). See `Extra/Guias/db_architecture.md`.
...
- To expose tools without separating sets, use a flat `mcp.tools` map, for example:
...
- Starts with `python app.py`; shows the final URL (`http://127.0.0.1:PORT/mcp`) and keeps the DuckDB connection in read-only mode when a compatible index exists.
- The web panel remains available when the RAG DB is missing or incompatible. RAG tools return a friendly rebuild-required error until an index is created.
...
- `requirements.txt` includes CPU libs, `fastapi`, `uvicorn`, and remote clients (`torch` CPU, `sentence-transformers`, `openai`, `requests`, etc.).
...
- Use the web panel or UI API rebuild endpoints to create a fresh RAG index from a sitemap or a URL file in `txt/`.
- Each rebuild creates the new RAG (`docs`/`chunks`/`metadata`) in a temporary DuckDB and only replaces the final DB if ingestion completes successfully. If crawler, embedding, insertion, indexes, or metadata fail, the previous index is preserved.
- The new index is populated only with documents obtained from the sitemap or URLs from the selected file in `txt/`, fully replacing previous RAG documents while leaving projects/items intact.

### MCP HTTP/JSON-RPC Compatibility

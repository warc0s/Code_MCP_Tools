...
# Filter the Pydantic warning generated by crawl4ai (Config in BaseModel).
        ...
        for extension in ("vss", "fts"):
            try:
                ...
                try:
                    ...
                    # Robustness: without network/extensions, continue without breaking startup.
                    logger.warning(
...
def _open_ro_connection(db_path: Path):
    ...
    for extension in ("vss", "fts"):
        try:
            ...
            logger.warning(
                "DuckDB: could not LOAD '%s' in the main connection; searches may degrade: %s",
                extension,
                exc,
            )
    return connection
...
def _rag_schema_available(db_path: Path) -> bool:
    ...
    try:
        ...
        logger.info("Could not check the RAG schema; assuming unavailable.")
        return False
    finally:
        connection.close()
...
def load_retriever(config: AppConfig) -> Tuple[Optional[Retriever], Optional[Any]]:
    ...
    if not rag_ready:
        connection = duckdb.connect(db_path.as_posix(), read_only=True)
        logger.info("DB exists but has no RAG schema; only general tables will be loaded.")
    ...
    except Exception as exc:  # pragma: no cover - protected DB opening
        logger.exception("Could not open the DB in read-only mode.")
        raise RuntimeError(f"Could not open the DB: {exc}") from exc
...
def _available_tools_from_config(config: AppConfig) -> Optional[list[str]]:
    ...
    def enabled_names(tool_map: Dict[str, Any]) -> list[str]:
        ...
        return [name for name, enabled in tool_map.items() if enabled and name in AVAILABLE_TOOL_NAMES]
...
AVAILABLE_TOOL_NAMES = {name for names in TOOL_GROUPS.values() for name in names}
...
def _get_db_overview(db_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Could not open the DB for overview.")
        return overview
    try:
        ...
        logger.info("Could not load the DB overview.")
    finally:
        try:
            connection.close()
        except Exception:
...
def _load_config_dict(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error("Could not read config.yaml: %s", exc)
        raise HTTPException(status_code=500, detail="Could not read config.yaml.") from exc
    if not isinstance(data, dict):
        raise HTTPException(status_code=500, detail="config.yaml must be a YAML mapping.")
    return data


def _persist_enabled_tools(enabled: list[str], state: AppState, config_path: Path) -> dict[str, Any]:
    """
    Persist the active tool list in config.yaml while preserving the existing structure (tools or tool_sets).
    Changes are not hot-applied; restart_required is marked so the UI can warn.
    """
    ...
    if mcp_section.get("tool_sets"):
        ...
        if selected_set and isinstance(selected_map, dict):
            base_keys = (set(selected_map.keys()) & set(available)) | set(enabled)
            new_map = {name: name in enabled for name in base_keys}
            mcp_section["tool_sets"][selected_set] = dict(sorted(new_map.items()))
        else:
            # Create a "rag" set when none existed.
        ...
        base_keys = (set(current_tools.keys()) & set(available)) | set(available) | set(enabled)
    ...
    try:
        ...
        logger.error("Could not write config.yaml: %s", exc)
        raise HTTPException(status_code=500, detail="Could not save config.yaml.") from exc

    # Refresh the in-memory config for later reads.
...
def _persist_settings(payload: Dict[str, Any], state: AppState, config_path: Path) -> dict[str, Any]:
    """
    Persist mode/model/reranker settings and UI settings (selected_project) in config.yaml.
    Only mode/model/reranker changes require restart/rebuild.
    """
    data = _load_config_dict(config_path)
    updated_any = False
...
def _run_rebuild_urls_file(filename: str, state: AppState):
    ...
    if not urls:
        raise ValueError("The file does not contain valid URLs.")
    ensure_extension_directory()
    embedder = EmbeddingProvider(state.config.embeddings, mode=state.config.main.mode)
...
def create_web_app(state: AppState, base_path: str) -> FastAPI:
    ...
    async def api_rebuild_sitemap(payload: Dict[str, Any]):
        ...
        if not url:
            raise HTTPException(status_code=400, detail="You must provide a sitemap URL.")
    ...
    async def api_rebuild_file(payload: Dict[str, Any]):
        ...
        if not filename:
            raise HTTPException(status_code=400, detail="You must specify a URL file.")
        try:
    ...
    async def api_restart():
        """
        Restart the service. Requires CONTAINER_NAME to restart the Docker container.
        There is no subprocess relaunch outside Docker.
        """
        container_name = os.getenv("CONTAINER_NAME")
        if not container_name:
            raise HTTPException(
                status_code=400,
                detail="CONTAINER_NAME is not defined; restart is only available in Docker environments.",
            )
        ...
        try:
            ...
            if stderr:
                ...
                detail += f" stdout: {stdout[:500]}"
            logger.error("Could not restart container %s: %s", container_name, stderr or stdout)
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"Could not load config.yaml: {exc}") from exc

    # Initialize the memory database (SQLite) for projects/items.
    try:
        bootstrap_memory_db(config.memory_database)
    except Exception as exc:
        raise SystemExit(f"Could not initialize the memory database: {exc}") from exc

    enabled_tools = _available_tools_from_config(config)
    retriever, connection = load_retriever(config)
    # ItemService now uses the dedicated SQLite memory DB.
    ...
    logger.info(
        "Web+MCP server at http://%s:%s (MCP at %s). Open http://%s:%s in your browser.",
        host,
        port,
        http_path,
        display_host,
